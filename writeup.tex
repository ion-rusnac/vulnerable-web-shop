...
Because the wallet's credit card processing system is intentionally hardcoded to decline all transactions, a standard user can never legitimately acquire a positive balance. The attacker must discover how the application tracks the user's wallet balance, exploit the underlying session management vulnerability to artificially inflate their balance, and subsequently complete a purchase. The ``flag'' or win-state is the successful creation of an order in the database and the rendering of the checkout success screen.
